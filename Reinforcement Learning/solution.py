...
class Actor(nn.Module):
    ...
    def forward(self, obs, act=None):
        ...
        logprob_a = None
        if act is not None:
            logprob_a = self._log_prob_from_distribution(pi, act)
        return pi, logprob_a
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        deltasGT = rews[:-1]
        self.ret_buf[path_slice] = discount_cumsum(deltasGT, self.gamma*self.lam)
...
class Agent:
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            pi, logp = self.actor(state)
            a = pi.sample()
            v = self.critic(state)
            _, logp = self.actor(state, a)
        if a.dim() == 0:
            a = a.item()
        return a, v, logp
    ...
    def get_action(self, obs):
        ...
        act = self.act(torch.as_tensor(obs, dtype=torch.float32))
        return act
...
def train(env, seed=0):
    ...
    for epoch in range(epochs):
        ...
        policy_loss = -(log_prob*tdres).sum() - 1e-5*torch.mean(-log_prob)
        ...
        for _ in range(100):
            ...
            loss = nn.MSELoss()
            value_loss = loss(value, ret)
            # value_loss = (0.5* data["tdres"].squeeze()**2).sum()
            value_loss.backward()
            critic_optimizer.step()            
