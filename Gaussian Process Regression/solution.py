...
class Model(object):
    ...
    def make_predictions(self, test_features: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        predictions = 0.97*gp_mean + 1.0*np.ones(gp_mean.shape[0])
        # predictions = multivariate_normal.rvs(mean = gp_mean, cov = gp_std, random_state = self.rng)
    ...
    def fitting_model(self, train_GT: np.ndarray,train_features: np.ndarray):
        ...
        """
        #X, _, y, _ = train_test_split(train_features,train_GT , train_size = 30000)
        X,y = train_features,train_GT
        kernel =RBF(0.022,length_scale_bounds="fixed")+ WhiteKernel(noise_level=0.01,noise_level_bounds="fixed")
        self.regressor = GaussianProcessRegressor(kernel=kernel).fit(X, y)
