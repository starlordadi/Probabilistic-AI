...
class MNISTNet(nn.Module):
    def __init__(self,
        ...
        self.layer2 = nn.Linear(300, 400)
        self.layer3 = nn.Linear(400, 350)
        self.layer4 = nn.Linear(350, out_features)
    ...
    def forward(self, x):
        ...
        x = F.dropout(x, p=self.dropout_p, training=self.training or self.dropout_at_eval )
        x = F.dropout(
...
class DropoutTrainer(Framework):
    def __init__(self, dataset_train,
        ...
        self.network = MNISTNet(in_features=28 * 28, out_features=10,  dropout_p=0.5,  dropout_at_eval=True)
        self.train_loader = torch.utils.data.DataLoader(
            dataset_train, batch_size=self.batch_size, shuffle=True, drop_last=True
        )
        self.optimizer = torch.optim.Adam(self.network.parameters(), lr=self.learning_rate)
    ...
    def predict_probabilities(self, x: torch.Tensor, num_sample=100) -> torch.Tensor:
        ...
        for i in range(num_sample):
            L.append(torch.argmax(self.network(x),dim=1).numpy())

        L = np.transpose(np.array(L))
        estimated_probability = []
        for image in L: #We have a series of runs of each image
            count = [0 for i in range(10)]
            for elt in image: #Let's iterate over the different predictions for each image
                count[elt]+=1
            
            estimated_probability.append([y/sum(count) for y in count])


        estimated_probability = torch.from_numpy(np.array(estimated_probability))
        ...
        assert estimated_probability.shape == (x.shape[0], 10)  
        return estimated_probability.type(torch.FloatTensor)
...
class EnsembleTrainer(Framework):
    def __init__(self, dataset_train,
        ...
        for i in range(self.num_models):
            network = MNISTNet(in_features=28*28, out_features=10)
            self.EnsembleNetworks.append(network)
            params = params + list(network.parameters())
        ...
        for network in self.EnsembleNetworks:   
            ...
            for _ in progress_bar:
                for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):
                    ...
                    # You need to calculate the loss based on the literature
                    loss = loss = F.cross_entropy(F.log_softmax(current_logits, dim=1), batch_y, reduction='sum')
    ...
    def predict_probabilities(self, x: torch.Tensor) -> torch.Tensor:
        ...
        estimated_prob_list = []
        for network in self.EnsembleNetworks:
            estimated_prob_list.append(F.softmax(network(x), dim=1).detach().numpy())

        estimated_probability = torch.from_numpy(np.mean(estimated_prob_list, axis=0))
